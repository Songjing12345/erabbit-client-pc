<template>
  <div class="account-box">
    ...
    <Form class="form" ref="formCom" :validation-schema="mySchema" v-slot="{errors}" autocomplete="off">
...
export default {
  ...
  setup () {
    ...
    // vee-validate校验的基本步骤
    // 1、导入Form, Field组件并进行注册，将form和input进行替换，需加上name属性用来指定将来的校验规则
    // 2、Field需要进行数据绑定，字段最好和后台接口的保持一致
    // 3、定义Field的name属性指定的校验规则函数，Form的validation-schema接受定义好的校验规则
    // 校验规则对象
    const mySchema = {
    ...
    // setup中获取组件实例proxy  通过实例对象调用$message方法
      ...
      if (valid) {
        try {
          ...
          if (!isMsgLogin.value) {
            ...
            // 1、发送验证码
            // 1.1、绑定发送验证码按钮的点击事件
            // 1.2、校验手机号，如果成功才发送短信，请求成功开启60秒倒计时，不能再次点击
            // 2、手机号登录
